...
**Training & Tracking:**  
PyTorch + MLflow for experiment tracking and model registry. Artifacts stored remotely (S3) to support a containerized server app.

**Model Serving:**  
Containerized FastAPI service that loads the MLflow‑registered model with alias `@champion`, or falls back to a model tagged `"basemodel"`.

**Batch Client:**  
Asynchronous client that:
- scans a date‑based folder
- micro‑batches images
- sends them to the server  
- writes JSONL ledger and daily Parquet  
- runs simple drift/imbalance analytics
